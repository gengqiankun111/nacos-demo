...
public class MyInterceptor implements HandlerInterceptor {
    @Autowired
    private JwtService jwtService;// = SpringContextUtil.getContext().getBean(JwtService.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        || request.getRequestURI().equals("/auth/login")|| request.getRequestURI().equals("/auth/login/md5")) {
            //jwtValid = true;
            return true;
        } else {
           jwtValid = jwtService.checkJwt(request.getHeader("authorization"));
        }
